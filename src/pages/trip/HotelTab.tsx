...
function HotelForm({ hotel, onSave, onDelete }: { hotel: Hotel; onSave: (h: Hotel) => void; onDelete?: () => void }) {
  ...
  return (
    <div>
      ...
      <div className="form-group"><label className="form-label">入住時間</label><input className="form-input" value={form.checkIn || ''} onChange={e => setForm({ ...form, checkIn: e.target.value })} placeholder="2026/04/09（四）14:00~23:30" /></div>
      <div className="form-group"><label className="form-label">退房時間</label><input className="form-input" value={form.checkOut || ''} onChange={e => setForm({ ...form, checkOut: e.target.value })} placeholder="2026/04/14（二）6:00~12:00" /></div>
