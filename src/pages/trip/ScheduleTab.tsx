...
export function ScheduleTab({ tripId, viewOnly }: Props) {
  ...
  const collapsedStorageKey = `schedule-collapsed-${tripId}`;
  ...
  }, [collapsedDays, collapsedStorageKey]);

  function getDayCollapseKey(day: ScheduleDay, dayIndex: number) {
    return day.date || `day-${dayIndex}`;
  }
  ...
  function toggleDay(day: ScheduleDay, dayIndex: number) {
    const key = getDayCollapseKey(day, dayIndex);
      ...
      {schedule.length === 0 ? (
        ...
        schedule.map((day, dayIndex) => (
          <div key={dayIndex} className="card mb-2">
            {(() => {
              const hasCollapsibleContent = day.activities.length > 0;
              const isCollapsed = hasCollapsibleContent
                ? collapsedDays[getDayCollapseKey(day, dayIndex)] || false
                : false;
          ...
          {selectedActivity.activity.booking?.platform && (
            <InfoRow
              ...
              value={
                <>
                  {selectedActivity.activity.booking.platform}
                  {selectedActivity.activity.booking.assignee && (
